...
################################################  
#Settings
run.taxon <- "ungulates" #"ungulates" "carnivores
...
#herbivore.size.cat <- c(-Inf, 1.03, 1.53, 2.01, 2.55, Inf) #Jenks natural breaks k =5
#herbivore.size.cat <- c(-Inf, 0.81, 1.26, 1.61, 2.04, 2.55, Inf) #Jenks natural breaks k =6
...
#predator.size.cat <- c(-Inf, 0.85, Inf) #Jenks natural breaks k =2
#predator.size.cat <- c(-Inf, -0.35, 0.23, 0.76, 1.51, Inf) #Jenks natural breaks k =5
#predator.size.cat <- c(-Inf, -0.35, 0.23, 0.72, 1.3, 2.04, Inf) #Jenks natural breaks k =6


pred.cat.name <- "allCateg" #"sub7kg"

do.pred.diet <- FALSE
diet.type <- c("hypercarnivore", "mesocarnivore") #"hypercarnivore" "mesocarnivore" "hypocarnivore"
if(do.pred.diet) pred.cat.name <- paste0(pred.cat.name,"_","hyper_meso","_")
...
#"/Users/emdoughty/Dropbox/Code/R/Results/repIntMaster__this.rank=species_timebin=2Mabins_start=66_end=0_SampleStandardized=TRUE_Reps=1000Jonathans_MBP.lan##------ Wed Apr 13 23:48:30 2022 ------##.Rdata"
"/Users/emdoughty/Dropbox/Code/R/Results/repIntMaster__this.rank=species_timebin=2Mabins_start=66_end=0_SampleStandardized=FALSE_Reps=10000Jonathans_MBP.lan##------ Thu Apr 14 20:19:28 2022 ------##.Rdata"
...
if(run.taxon == "carnivores")
{
  ...
  if(do.pred.diet)
  {
    ...
    measure.mat <- measure.mat[measure.mat$taxon %in% pred.diet$MASTER_LIST,]
    measure.mat$diet <- pred.diet[pred.diet$MASTER_LIST %in% measure.mat$taxon, "PPP_diet_2"]
    
    measure.mat <- measure.mat[measure.mat$diet %in% diet.type,]
  } 
}
...
if(interval.type == "bins")
{
  ...
  save.path.bins <- paste0(int_length,"Ma",interval.type, "_start=", max(intervals),"_end=",min(intervals))
}
#######################
...
#repIntOccs <- lapply(repIntOccs, function(this.rep) {this.rep[rownames(intervals)]}) 
